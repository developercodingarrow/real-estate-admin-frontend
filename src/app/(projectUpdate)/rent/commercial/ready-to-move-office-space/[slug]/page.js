...
import RCReadyToMoveOfficeWrapper from "../wrapper";
import NotDataFound from "@/src/components/errorpages/NotDataFound";

export default async function RCReadyToMoveOfficepage({ params }) {
  const { slug } = params;
  let data;
  try {
  ...
  return (
    <div>
      <RCReadyToMoveOfficeWrapper data={data} slug={slug} />
    </div>
  );
}
