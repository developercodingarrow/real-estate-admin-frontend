...
import UpdateSCCommerercialWrapper from "../wrapper";

export default async function UpdateSCCommerercialProjectPage({ params }) {
  const { slug } = params;
  let data;
  try {
  ...
  return (
    <div>
      <UpdateSCCommerercialWrapper data={data} slug={slug} />
    </div>
  );
}
